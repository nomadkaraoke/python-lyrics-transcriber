...
class KaraokeComposer:
    ...
    def __init__(
        ...
        for ci, lyric in enumerate(self.config.lyrics):
            ...
            for li, (line, singer, line_image, line_mask) in enumerate(
                ...
                y = lyric.row * CDG_TILE_HEIGHT + (
                    (li % lyric.lines_per_page) * lyric.line_tile_height *
                    CDG_TILE_HEIGHT)
        ...
        max_height = max(line.image.height for lyric in self.lyrics
                         for line in lyric.lines)
        line_offset = (self.max_tile_height * CDG_TILE_HEIGHT -
                       max_height) // 2
        logger.debug(
    ...
    def _set_draw_times(self):
        ...
        for lyric in self.lyrics:
            ...
            # syllable
            first_syllable = next(
                iter(syllable_info for line_info in lyric.lines
            ...
            for last_wipe, wipe in it.pairwise(
                    syllable_info for line_info in lyric.lines
                    for syllable_info in line_info.syllables):
    ...
    def _set_draw_times_page(
        ...
        if minimum_time < 32:
            logger.warning("not enough bandwidth to clear screen on lyric "
    ...
    def _set_draw_times_line_delayed(
        ...
        last_line_start_offset = lyric.lines[
            last_wipe.line_index].syllables[0].start_offset
    ...
    def compose(self):
        ...
        current_time = (self.writer.packets_queued - self.sync_offset -
                        self.intro_delay)
        ...
        while any(state.line_draw < len(times.line_draw) or state.line_erase <
                  len(times.line_erase) or state.syllable_line < len(
                      lyric.lines) or state.draw_queue or state.highlight_queue
                  for lyric, times, state in zip(
                      self.lyrics,
                      self.lyric_times,
                      lyric_states,
                  )) or (composer_state.instrumental < len(
                      self.config.instrumentals)):
            for lyric, times, state in zip(
                    self.lyrics,
                    self.lyric_times,
                    lyric_states,
        ...
        padding_before_outro = ((end - OUTRO_DURATION) -
    ...
    def _compose_lyric(
        ...
        if should_erase_this_line:
            ...
            logger.debug(f"t={self.writer.packets_queued}: erasing lyric "
        ...
        if should_draw_this_line:
            ...
            logger.debug(f"t={self.writer.packets_queued}: drawing lyric "
        ...
        if state.syllable_line < len(lyric.lines):
            syllable_info = (lyric.lines[state.syllable_line].syllables[
                state.syllable_index])
            ...
            if instrumental.wait:
                ...
                # If syllable was found
                    ...
                    first_syllable = lyric.lines[
                        last_syllable.line_index].syllables[0]
                    # If this line is being actively sung
                    if current_time >= first_syllable.start_offset:
                        # If this is the last syllable in this line
                        if last_syllable.syllable_index == len(lyric.lines[
                                last_syllable.line_index].syllables) - 1:
                            instrumental_time = 0
                            if times.line_erase:
                            ...
                            if not instrumental_time:
                                ...
                                instrumental_time = (last_syllable.end_offset +
                                                     450)
                        else:
                            logger.debug(
        ...
        if should_instrumental:
            ...
            if instrumental.wait:
                ...
                logger.debug("this instrumental did not wait for the previous "
                             "line to finish")
        ...
        draw_groups: list[list[CDGPacket
                               | None]] = [[None] * self.max_tile_height
        ...
        self.lyric_packet_indices.update(
            range(
                self.writer.packets_queued,
                self.writer.packets_queued +
                len(list(it.chain(*highlight_groups, *draw_groups))),
            ))
        ...
        for group in intersperse(highlight_groups, draw_groups):
            for item in group:
                ...
                self.writer.queue_packet(
                    next(
                        iter(st.draw_queue.popleft() for st in lyric_states
                             if st.draw_queue), no_instruction()))
    ...
    def _compose_highlight(
        ...
        column_group_length = (
            ...
            self.max_tile_height) * len(self.lyrics)
        # Calculate the number of column updates for this highlight
        columns = ((end_offset - start_offset) //
                   column_group_length) * self.config.highlight_bandwidth
        ...
        # updates
            ...
            syllable_text = "".join(f"{{{syll.text}}}" if si ==
                                    syllable.syllable_index else syll.text
                                    for si, syll in enumerate(lyric.lines[
                                        syllable.line_index].syllables))
        ...
        return [
            ...
            for edges in it.pairwise([left_edge] + highlight_progress +
                                     [right_edge])
        ]
    ...
    def _compose_instrumental(
        ...
        text_images = render_lines(
            ...
            stroke_width=(self.config.stroke_width
                          if instrumental.stroke is not None else 0),
            stroke_type=self.config.stroke_type,
        )
        ...
        match instrumental.text_placement:
            ...
            case (TextPlacement.MIDDLE_LEFT
                  | TextPlacement.MIDDLE
                  | TextPlacement.MIDDLE_RIGHT):
                text_y = ((CDG_SCREEN_HEIGHT - text_height) //
                          2) // CDG_TILE_HEIGHT * CDG_TILE_HEIGHT
                # Add offset to place text closer to middle of line
                text_y += (line_height - max_height) // 2
            case (TextPlacement.BOTTOM_LEFT
                  | TextPlacement.BOTTOM_MIDDLE
                  | TextPlacement.BOTTOM_RIGHT):
        ...
        if instrumental.image is None:
            ...
            padright = -(instrumental.x +
                         background_image.width) % CDG_TILE_WIDTH
            padtop = instrumental.y % CDG_TILE_HEIGHT
            padbottom = -(instrumental.y +
                          background_image.height) % CDG_TILE_HEIGHT
            logger.debug(
            ...
            logger.debug("instrumental background image packed in "
        ...
        self.writer.queue_packets([no_instruction()] *
    ...
    def _compose_intro(self):
        ...
        logger.debug("intro background image packed in "
                     f"{len(list(it.chain(*packets.values())))} packet(s)")
        ...
        end_time = DURATION
        self.writer.queue_packets([no_instruction()] *
                                  (end_time - self.writer.packets_queued))

        first_syllable_start_offset = min(syllable.start_offset
                                          for lyric in self.lyrics
                                          for line in lyric.lines
        ...
        logger.info(f"should delay intro by {self.intro_delay} frame(s)")
...
winslowjosiah@gmail.com""",
        ...
        self.writer.queue_packets([no_instruction()] *
                                  (end - self.writer.packets_queued))
    ...
    def _load_image(
        ...
        image_rgba = Image.open(file_relative_to(
            image_path, self.relative_dir)).convert("RGBA")
        ...
        image.putpalette([
            ...
            0x11 * round(v / 0x11) for v in image.getpalette()
        ])
        image = image.quantize()
        ...
        logger.debug(f"palette: {list(it.batched(image.getpalette(), 3))!r}")
        ...
        mask.putdata([
            0 if pixel >= 128 else 255 for pixel in image_rgba.getdata(band=3)
        ])
    ...
    def create_ass(self):
        ...
        for ci, (lyric,
            ...
            for li, line in enumerate(lyric.lines):
                ...
                while (instrumental < len(self.instrumental_times)
                       and (cdg_to_sync(self.instrumental_times[instrumental])
                            <= line_draw_time)):
                    instrumental += 1

                # Get intended erase time of line, if possible
                if times.line_erase:
                    line_erase_time = (cdg_to_sync(times.line_erase[li]) +
                                       offset)
                ...
                # being cleared by page instead of being erased
                    ...
                    next_page_li = (li // lyric.lines_per_page +
                                    1) * lyric.lines_per_page
                    ...
                    if next_page_li < len(lyric.lines):
                        ...
                        # drawn
                        line_erase_time = (
                            cdg_to_sync(times.line_draw[next_page_li]) +
                            offset)
                    # If there is no next page
                        ...
                        # XXX This is hardcoded.
                        line_erase_time = (
                            cdg_to_sync(line.syllables[-1].end_offset) +
                            offset + 200)
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        ...
        epilog=("For a description of the config format, visit "
