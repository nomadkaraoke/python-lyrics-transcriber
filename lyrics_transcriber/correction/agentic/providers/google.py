...
class GoogleProvider(BaseAIProvider):
    """Google provider wrapper delegating to LiteLLMBridge."""

    def __init__(self, model: str = "gemini-2.5-pro", config: ProviderConfig | None = None):
        self._delegate = LiteLLMBridge(model=model, config=config)

    def name(self) -> str:
        return f"google:{self._delegate._model}"  # type: ignore[attr-defined]
